...
- Records a non-negative critical-error count per candidate.
- Selects exactly one preferred candidate per block.

The confidential analysis command requires the blinded ZIP, separate key, and all response files. It validates hashes and schemas before unblinding, then reports per-system rubric means, preference counts/rates with 95% Wilson intervals, critical errors per block, and Fleiss' kappa for preference agreement.
